...
| AI observabilidade | Custo por chamada no ledger `AiUsageEntries`, visível em `/ai/usage` (`GET /api/v1/ai/usage`, `ai.agent.read`). Com `OpenTelemetry:EnableTraces=true` cada chat produz spans GenAI (`invoke_agent` → `chat` → `execute_tool`) filhos do span HTTP; **o template não traz exportador** — acrescente `OpenTelemetry.Exporter.OpenTelemetryProtocol` e o endpoint do seu collector antes de produção, senão os spans ficam só em processo. |
