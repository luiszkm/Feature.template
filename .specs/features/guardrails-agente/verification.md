...
**Verdict**: PASS
**Profile**: ui
**Diff range**: b888b3e..edf16cd (round-2 fix range a47ffa5..edf16cd)
**Round**: 2 - scoped
**Verifier**: independent sub-agent (author != verifier)

Round 1, verified at a47ffa5, returned FAIL. Two things were unproven: the start of AC 20's daily
window (00:00 UTC) and the `AiQuota` decision at its own layer. Commit edf16cd fixes them:

- `AiQuota` now takes a `TimeProvider` (`src/Api/Features/Ai/AiRateLimit.cs:65,80`), registered with `TryAddSingleton(TimeProvider.System)` at `src/Api/Features/Ai/AiModule.cs:34`.
- C21 gains two layer proofs. The comparisons `429` test now asserts `detail`, and C7 now asserts the exact log message.
- No claim text changed.

Round 2 scope:

- All proofs re-run in full at edf16cd.
- New faults on every surface the fix touched or created.
- The quota Coverage row and the unmet Test policy row are recomputed.
- Citations are refreshed for the touched files: `AiRateLimit.cs`, `AiModule.cs`, `AiRateLimitTests.cs` and `AgentLoopGuardrailTests.cs`.
- Everything else is carried from a47ffa5 and marked as such. The fix did not touch the interface, so step 1 is carried.

## Binding sources

_carried from a47ffa5. The fix diff touches no file under `src/web/`._
...
_verified at edf16cd. The proofs re-ran in full. Citations are refreshed for the touched files, and the other citations are carried from a47ffa5 because those files are unchanged in a47ffa5..edf16cd._

Proof runs, all at edf16cd:
- **A**: `dotnet test tests/Api.Tests --filter "<42 FullyQualifiedName~ alternations>" --logger trx`. Exit 0, 52 passed, 0 failed. The trx lists each of the 44 names as Passed; the 42 from round 1 plus the 2 new C21 proofs, with Theory cases counted individually.
...
- **W**: `cd src/web && npx -y node@24.15.0 node_modules/@angular/cli/bin/ng.js test --no-watch --include src/app/features/ai/chat.spec.ts --filter "429 mostra o detail e mantem a mensagem|400 mostra o erro de message"`. Exit 0, 2 passed, 7 skipped, run in the real tree only. The Node substitution is the environment workaround noted in checks.md.
...
| C21 | quota=2 -> chat 200 then 429 title/detail, 1 LLM call; same 429 on comparisons | A: all 4 passed | `tests/Api.Tests/Ai/AiRateLimitTests.cs:116-121` - `OK`, `TooManyRequests`, title `"AI quota exceeded"`, detail `"Limite diário de tokens de IA do tenant atingido."`, `Assert.Single(llm.Requests)`; `:136-140` comparisons `TooManyRequests`, title, detail (`:139`), `Assert.Single`; `:153` - `Assert.Equal(new DateTime(2026, 9, 23, 0, 0, 0, DateTimeKind.Utc), ledger.LastSince)` for a clock at 2026-09-22T23:30-03:00; `:162-167` - spent 9 of 10 passes, spent 10 `Assert.ThrowsAsync<TooManyRequestsException>` with `error.Title` / `error.Message` literals | PASS |
| C22 | SumTokensSince: current tenant only, `>= since` | A: passed | `tests/Api.Tests/Ai/AiRateLimitTests.cs:186-187` - `Assert.Equal(7, ...SumTokensSinceAsync(UtcNow.AddMinutes(-1)))` (the other tenant's 200 excluded); `Assert.Equal(0, ...AddMinutes(1))` | PASS |
| C23 | quota 0 -> ledger not queried | A: passed | `tests/Api.Tests/Ai/AiRateLimitTests.cs:193-194` - `Assert.Equal(0, await LedgerQueriesForOneChatAsync(quota: 0))`; `Assert.Equal(1, ...quota: 1_000)`; `:235` reply `"ok"` | PASS |
| C24 | openapi declares 429 on both routes; guard covers `AiRateLimitPolicy` | R + E passed | `tests/ArchitectureTests/OpenApiContractTests.cs:127-128` - filter `Contains("AuthRateLimitPolicy") or Contains("AiRateLimitPolicy")`; `:153` - `Assert.True(undeclared.Count == 0, ...)`; `tests/E2ETests/Common/OpenApiDocumentTests.cs:60` - `Assert.True(committed == generated, ...)`; `src/Api/openapi.json:1397,1487` `"429"` under `/api/v1/ai/chat` (1340) and `/api/v1/ai/comparisons` (1410) | PASS |
...
_quota row verified at edf16cd. The other rows are carried from a47ffa5, whose authority the fix did not touch. The startup-config row is refreshed for the `AiModule.cs` line shift._
...
| daily quota, AC 20-21 (6) | plan AC 20-21 + Assumptions "soma desde 00:00 UTC"; `AiRateLimit.cs:77,80,82` at edf16cd | off (`<= 0`) C23 · at limit (`>=`) at the layer C21 (`Quota_ShouldThrowAtLimit_AndPassBelowIt`: 9 passes, 10 throws) · chat over HTTP C21 · comparisons over HTTP C21 · window start = 00:00 UTC of the current UTC day C21 (`Quota_ShouldCountFromUtcMidnight_OfCurrentDay`, a local evening crossing the UTC date) · tenant/`since` filter C22 | - |
...
| startup config (2 assemblies) | read directly: `Program.cs:12` `AddFeatureModules()` -> `FeatureModulesConfiguration.cs:16` `AddAiModule()` -> `AiModule.cs:23` `AddPolicy<string, AiRateLimitPolicy>`, `AiModule.cs:33` `AddSingleton<IContentGuard, AllowAllContentGuard>`, `AiModule.cs:34` `TryAddSingleton(TimeProvider.System)`; `TestServiceFactory.cs:32` `services.AddAiModule()` | real Program: C13, C18 (HTTP) · TestServiceFactory: C12 default, C16 | - |
...
uses two tenants. That is a non-blocking note, because C20 as written is met (carried from a47ffa5).

## Test policy rows

_row 1 re-judged at edf16cd, because it was unmet in round 1 and classifies `AiRateLimit.cs`, which the fix touched. Rows 2 and 3 are carried from a47ffa5; row 2 classifies `AgentLoop.cs`, which is unchanged, and only its test assertion (C7) got stronger._

| Row | Files it classifies | Required proof | Expectation met |
| --- | --- | --- | --- |
| Decides, crosses the HTTP boundary (validator, message guard, quota, limiter) | `ChatAi.cs` (validator, guard call), `CompareModels.cs` (guard call), `AiRateLimit.cs` (`AiQuota`, `AiRateLimitPolicy`) | boundary + own layer; status and body at the boundary; one case per decision-table row at the layer | yes - validator: C1 HTTP + C1-C4 layer · message guard: C13/C17 HTTP + C13 handler · limiter: C18 HTTP + C20 layer · `AiQuota`: C21 HTTP (title + detail on both routes, `AiRateLimitTests.cs:119-120,138-139`) + layer, one case per row: off C23, window start C21 `:153`, below limit and at limit C21 `:162-167` |
...
Swept rows that cite existing constraints, re-read against the code (carried from a47ffa5, with the soft-quota line refreshed):
...
- concurrency, "soft quota, documented": the doc comment is at `AiRateLimit.cs:70-74` and is unchanged by the fix.
...
_Round 2 faults verified at edf16cd. Round 1 faults carried from a47ffa5; the surfaces they hit
are unchanged in the fix diff._

Round 2 ran in a scratch `git worktree` at edf16cd. The real-tree porcelain was empty before the
run and empty after. The worktree has been removed. I ran no vitest in the worktree, and no
`.angular/cache` leak occurred.

| Mutation | Location | Killed |
| --- | --- | --- |
| (r2) window start `clock.GetUtcNow().UtcDateTime.Date` -> `DateTime.MinValue` (lifetime) | `src/Api/Features/Ai/AiRateLimit.cs:80` | yes - `Quota_ShouldCountFromUtcMidnight_OfCurrentDay` failed |
| (r2) window start -> `clock.GetUtcNow().UtcDateTime.AddHours(-24)` (rolling 24h) | `src/Api/Features/Ai/AiRateLimit.cs:80` | yes - `Quota_ShouldCountFromUtcMidnight_OfCurrentDay` failed |
| (r2) `spent >= limit` -> `spent > limit` | `src/Api/Features/Ai/AiRateLimit.cs:82` | yes - `Quota_ShouldThrowAtLimit_AndPassBelowIt` failed |
| (r2) `LogError(ex, "Tool {ToolName} failed", toolCall.Name)` -> `LogError(ex, "Tool failed")` | `src/Api/Features/Ai/AgentLoop.cs:93` | yes - `RunAsync_ShouldReturnToolFailed_AndLogError_WhenToolThrows` failed |
| (r2) removed `TryAddSingleton(TimeProvider.System)` | `src/Api/Features/Ai/AiModule.cs:34` | yes - `Handle_ShouldNotQueryLedger_WhenQuotaIsZero` failed at resolution. `Chat_ShouldReturn429_WhenDailyTokenQuotaReached` stayed green: the ASP.NET host resolves a `TimeProvider` without this line, so the registration is load-bearing only outside the web host |
| (r1) validator role set `{user, assistant}` -> `{user, assistant, tool}` | `src/Api/Features/Ai/ChatAi.cs:42` | yes - `Validator_ShouldFail_WhenHistoryRoleIsNotUserOrAssistant(role: "tool")` failed |
| (r1) removed `catch (UnauthorizedAccessException)` arm | `src/Api/Features/Ai/AgentLoop.cs:86-90` | yes - `RunAsync_ShouldReturnPermissionDenied_WhenToolThrowsUnauthorized` failed |
| (r1) closing-tag regex drops `RegexOptions.IgnoreCase` | `src/Api/Features/Ai/ContentGuard.cs:56` | yes - `RunAsync_ShouldNeutralizeClosingTag_InsideToolOutput` failed |
| (r1) partition key `tenantId.ToString("N")` -> constant `NoTenantKey` | `src/Api/Features/Ai/AiRateLimit.cs:61` | yes - `Policy_ShouldPartitionByTenant` failed |
| (r1) chat error drops `fieldError(problem, 'message') ??` | `src/web/src/app/features/ai/chat.ts:144` | yes - `400 mostra o erro de message` failed |

The new comparisons-detail assertion (`AiRateLimitTests.cs:139`) got no mutant of its own. The
only code producing that detail is the shared `AiQuota.ExceededDetail`, which the chat assertion
(`:120`) and the layer assertion (`:167`) already pin. A comparisons-only mutant would need code
that does not exist.
...
_verified at edf16cd._

- `dotnet test tests/Api.Tests` (44-name filter): 52 passed, 0 failed.
...
- `ng test chat.spec.ts` (2 names, real tree): 2 passed, 0 failed.
