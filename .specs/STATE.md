...
| AD-013 | Workflows de agentes (`.specs/features/workflows-agente`, 2026-09-23): execução em segundo plano por polling da base - `WorkflowRunner : BackgroundService` lê `AiWorkflowRuns` `Queued` com `IgnoreQueryFilters()`, reclama com concurrency token (`Queued`→`Running`) e marca `Interrupted` o que passa de `Ai:Workflows:MaxRunMinutes`. É o padrão para o próximo job de fila; `Channel<T>` em memória e Hangfire/Quartz rejeitados | active |
| AD-014 | Um trabalho que corre tools fora do pedido HTTP guarda no registo o principal de quem o lançou (`userId`, roles, permissões) e o worker expõe-no por `ICurrentUserAccessor` no scope; permissões revogadas a meio só deixam de valer no fim do run (≤ 30 min). Conta de serviço com todas as permissões rejeitada | active |
